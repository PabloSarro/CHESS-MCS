# DeploymentSim Application

This deployment was auto-generated by the F' utility tool.

## Building and Running the DeploymentSim Application

In order to build the DeploymentSim application, or any other F´ application, we first need to generate a build directory. This can be done with the following commands:
...
The next step is to build the DeploymentSim application's code.
```
fprime-util build
```
...
To run the ground system without starting the DeploymentSim app:
```
...
```
cd DeploymentSim/build-artifacts/<platform>/bin/
./DeploymentSim -a 127.0.0.1 -p 50000
```
